...
class  Input
{
    ...
    static function ValidPassword($val)
    {
        if (!is_string($val) || strlen($val) >= 1000 || strlen($val) < 6) {
            self::throwError('Invalid String', 902);
        }
        return $val;
    }

    static function strXSS($val)
    {
        if (!is_string($val) || strlen($val) >= 1000) {
            self::throwError('Invalid String', 902);
        }
    ...
    static function bool($val)
    {
        if (filter_var($val, FILTER_VALIDATE_BOOLEAN)) {
            self::throwError('Invalid BOOL', 906);
        }
        return $val;
    }
