function UserAlertError(title, text) {
    sweetAlert(title, text, "error");
}

function UserAlertErrorImage(title, text) {
    swal({
        title: title,
        text: text,
...
function UserAlertSuccessVI(title, text) {
    swal(title, text, "success")
}

function DeleteUserAlert(title, text, row_id, page) {
    swal({
        title: title,
        text: text,
        type: "warning",
        ...
        function () {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response == 'Deleted') {
                        if (page == 'Manage') {
                            swal("Deleted !!", "Hey, the User has been deleted !!", "success");
                            $('#' + row_id).hide();
                        }
                        else if (page == 'Edit') {
                            location.href = "../users/manage.php";
                        }
                    }
                }
            });
...
function DeleteScanAlert(title, text, row_id, page) {
    swal({
        title: title,
        text: text,
        type: "warning",
        ...
        function () {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response == 'Deleted') {
                        if (page == 'History') {
                            swal("Deleted !!", "Hey, the Scan has been deleted !!", "success");
                            $('#' + row_id).hide();
                        }
                        else if (page == 'View') {
